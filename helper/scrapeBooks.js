...
const { writeInExcel } = require("./writeInExcel");

exports.scrap_books = async (book, books) => {
  ...
  try {
    ...
    if (searchResults.length === 0) {
      ...
      if (bestMatches.length === 0) {
        ...
        for (const oneBook of finalResults) {
          ...
          // get all the prices of the final books
        ...
        // finding minimum of all the prices
        ...
        for (let f = 0; f < 1; f++) {
          ...
          // getting all the details of the final book
          const newTarget = await browser.waitForTarget(
            (target) => target.opener() === pageTarget
          ); //check that you opened this page, rather than just checking the url
          const newPage = await newTarget.page(); //get the page object
    ...
    // Close the browser
    await browser.close();
    // write the details in the excel file
    writeInExcel(books);
  }
};
